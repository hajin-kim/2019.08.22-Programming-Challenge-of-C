...
Contacts * GetData(int * len)
{
	FILE * data = fopen("28-7 data\\mybook.txt", "rt");
	Contacts * book = (Contacts *)malloc(*len);
	...
	{
		book = (Contacts *)realloc(book, ++*len * size);
		fscanf(data, "%s %s ", book[*len - 1].name, book[*len - 1].tel);
	}
...
int main(void)
{
	...
	output = fopen("28-7 data\\mybook.txt", "wt");
